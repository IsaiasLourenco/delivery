    ...
    <ol class="list-group mg-t-6">
        ...
        if ($total_reg > 0) {
            for ($i = 0; $i < $total_reg; $i++) {
                ...
                $checkVar = $pdo->prepare("SELECT COUNT(*) FROM variacoes 
                                           WHERE produto = :produto AND ativo = 'Sim'");
                $checkVar->execute([':produto' => $id_produto]);
                $tem_variacao = $checkVar->fetchColumn() > 0;

                // Verifica se tem adicionais ou ingredientes
                $checkExtras = $pdo->prepare("SELECT 
                                             (SELECT COUNT(*) FROM adicionais WHERE produto = :produto AND ativo = 'Sim')
                                           + (SELECT COUNT(*) FROM ingredientes WHERE produto = :produto AND ativo = 'Sim')");
                $checkExtras->execute([':produto' => $id_produto]);
                $tem_extras = $checkExtras->fetchColumn() > 0;

                if ($tem_variacao) {
                    $link = "variacoes.php?url=$url_produto";
                } elseif ($tem_extras) {
                    $link = "adicionais.php?url=$url_produto";
                } else {
                    $link = "add-produto-direto.php?produto=$id_produto";
                }


        ?>
                <a href="<?php echo $link ?>" class="link-neutro <?php echo $mostrar ?>">
